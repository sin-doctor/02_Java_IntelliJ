...
public class Menu {
    ...
    private double price;   // 메뉴 가격
        ...
        // 메서드 기본 필수 Setter Getter toSting
    ...
    // 현재 객체 필드와 전달받은 객체의 필드가 모두 같을 경우 true, 아니면 false를 보여줌

    @Override
    public boolean equals(Object o) {
        // 들어온 값이 Menu 타입인지 확인 메뉴 타입이 같은 것인지 확인하고 비교할 이유가 없기 때문에 돌려보내기
        if (!(o instanceof Menu menu)) return false;
        // 만약에 메뉴에 들어올 값이 맞음
        // id 값이 같은지 확인         가격이 같은지 확인              메뉴이름이 같은지 확인
    ...
    // equals
//    두 값이 논리적으로 같은지 비교
// 재정의 해서 사용할 경우 객체 안에서 특정 필드 값이 같을 때 논리적으로 같다고 정의하고 비교 내용을 구현
// equals 에서 세부적으로 같은지를 확인
