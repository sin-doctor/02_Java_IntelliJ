...
public class HashSetPre1 {
   ...
   Set (집합, 주머니)
     ...
     */
    /*
     method1() 과일들을 넣고 중복허용되는지 확인
     */
    public void method1(){
        HashSet set = new HashSet(); // 자료형 제한없이 String int 개발자가 만든 자료형... 다양한 자료형 모두 ok
        set.add("점심");
        set.add(100);
        set.add(true);
        set.add(3.14);
    ...
    // method2 HashSet을 이용해서 자료형이 String 과일 추가
    public void method2(){
        ...
        // 결과를 보기 위해서는 System.out.print 로 출력해서 확인해야함!
        ...
        System.out.println("set 리스트 출력해서 목록이 비어있는지 확인 : "+ set);



    }

    // method3 HashSet을 이용해서 자료형이 int를 이용해서 10 200 3000 500 40 추가
    ...
    public void method3(){
        ...
        //set 에는 index라는 개념이 존재하지 않기 때문에
    ...
    // 일반 for문 경우 index 가 존재하지 않기 떄문에 사용 제한
    public void method4(){
        ...
        for (String s : strList) {
            System.out.println(s); // 중복이 허용되지 않음
    ...
    public void method5(){
        ...
        // HashSet 목록을 담고있는 str 변수명을 가져와서 반복하겠다 설정
        Iterator<String> it = str.iterator();
        while(it.hasNext()){ // 만약에 hasNext() = 다음값이 존재한다면 true
            System.out.println(it.next());
            // it 에는 현재 str에서 꺼내온 값이 들어있음 값을 꺼냈으면 next 다음 값을 준비해서 it 넣는 것
        }
        /*
            Iterator = 컬렉션을 순차적으로 회전하는 객체
                        (HashSet 이외에도 ArrayList 등 Collection으로 작성된 객체들에게 모두 사용 가능)
           .iterator() = 반복을 원하는 변수명 뒤에 작성한 후 반복을 하겠다는 설정
                         값에다가 순차적으로 접근

           .hasNext() = 반복할 수 있는 값이 남아있는지 확인
                        결과 : true = 아직 반복할 요소가 남아있음
                              false = 더 이상 반복할 요소(=값)가 없음
    ...
    public void iterLotto(){
        ...
        HashSet<Integer> lottoNumners = new HashSet<>();

        // 2. 중복없이 6개의 번호 생성 for문 이용
        // 로또숫자들이 총 0부터 5까지 6개 이하일 때만 for문을 반복
        for (int i = 0; lottoNumners.size() < 6; i++){
            // 어떤 숫자를 추가할 것이냐면 Math.random으로 가져온 숫자를 사용
            // Math.random() = 0.0 ~ 1.0 사이의 소수를 출력 0.0000000001 ~ 0.99999999999 사이 소수 출력
            // Math.random() * 45 = 45.2342313123123213 앞에 (int) 를 붙여주면 . 뒤에 있는 소수들이 버려짐
            // (int) (Math.random() * 45) 소수점이 버려졌기 때문에 0부터 44까지의 숫자를 만듬
            // 로또에는 0이라는 숫자가 없고, 45까지 숫자가 존재하기 때문에 모든 수 뒤에 +1을 붙여줌으로써
            // 0~45자리 랜덤숫자를 만들수 이ㅏㅆ음
            //          Math.random()*45)+1;
            int num =(int)(Math.random()* 45)+1;
            lottoNumners.add(num);
        }
        System.out.println("lottoNumners 출력 : "+ lottoNumners);

        Iterator<Integer> it = lottoNumners.iterator();
    ...
    // for-each(향상된 for문)을 이용해서 로또 완성하기
