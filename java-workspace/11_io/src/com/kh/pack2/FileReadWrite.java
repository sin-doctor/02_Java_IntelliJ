...
public class FileReadWrite {
    ...
    public void method1(){
        System.getProperty("user.home"); //c/user/user1 + /바탕화면
        ...
        try {   // 파일 안에 글을 작성하는 건 문제가 많을 수 있기 때문 예외상황 감지하는 try - catch 사용
            FileWriter writer = new FileWriter(file); // File 안에 Writer 글자 작성

            String content = "안녕하세요. 반갑습니다. 환영해요.";

            writer.write(content); // 파일쓰기 기능으로 파일안에 글자를 쓸 기능 호출 () 안에 작성할 내용 작성
    ...
    // true를 작성하지 않으면 기본적으로 false 상태로 덮어쓰기 진행
    ...
    public void method2(){
        ...
        try {
            FileWriter fw = new FileWriter(file,true); // 파일이 존재할 경우 이어쓰기모드
            String content2 = "\n 이어서 작성한 내용입니다.";
            fw.write(content2);
            fw.close(); // 이어 작성 완료 후 파일 닫기
            System.out.println("파일 이어서 작성하기 완료");
        } catch (IOException e) {
            System.out.println("이어서 작성하는것을 실패했습니다!");
        }finally {
