...
public class FileService {
    ...
    public void method3(){
        ...
        // file 에서 C:/Users/user1/Downloads 위치까지 설정해준 것
        // \ 를 되도록이면 / 변경해서 사용 Ctrl + F \ 입력 / 모두 바꾸기
        // 왜냐하면 \n 줄바꿈이나 \ 특수기호 사용과 같은 자바에서 특수문자 규칙
        File file = new File(System.getProperty("user.home") + "/Downloads");

        // 특정 폴더 위치에 있는 파일과 폴더들을 배열 형태로 가져올 것

        File[] files = file.listFiles(); // C:/Users/user1/Downloads 에 잇는 것들 모두 목록형태로 가져오기

        // 향상된 for문을 이용해서 출력
        for (File f : files) {  // 일반 for문 아니라 향상된 for 문
            System.out.println("파일이름 : "+f.getName()); // 파일이름들을 가져올 수 있음
            System.out.println("파일크기 :"+f.length()+"바이트");
            System.out.println("마지막 수정 날짜 :"+f.lastModified());
            // 마지막 수정 날짜 : 1731660312497 -> 1970년 1월 1일 부터 지난 일자
            ...
            String formatDate2  = sdf.format(f.lastModified()); // 1731660312497 -> sdt 형식으로 변경해서 문자열로 전달
            // Cannot format given Object as a Date 파일의 마지막 수정 날짜를 최대한 큰 공간으로 가져온 뒤 Date
